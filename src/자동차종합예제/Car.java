...
public abstract class Car { //객체를 만들 필요가 없기 때문에 추상 클래스로 생성
    ...
    // 이동 횟수 구하기 : 외부의 입력 값으로 승객의 수가 필요
    ...
    public int getRefuelCnt(int dist, int moveCnt) {
        if (((dist * moveCnt) / fuelEff) % fuelTank != 0)
            return (((dist * moveCnt) / fuelTank) / fuelTank) + 1;
        return  ((dist * moveCnt) / fuelTank) / fuelTank;
    }
