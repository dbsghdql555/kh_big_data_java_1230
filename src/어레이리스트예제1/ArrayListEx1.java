...
// Vector : ArrayList 와 동일 특성을 가짐, 단 멀티 쓰레드 환경을 위해 동기화 지원하기 때문에 안정적이나 ArrayList 보다 속도는 느림
// LinkedList : 물리적으로 연속된 공간이 아님, 참조로 다음 저장 위치를 가리킴. 삽입/삭제가 매우 빠름, 검색은 느림
public class ArrayListEx1 {
    public static void main(String[] args) {
        String [] data = {"138", "145", "122", "155", "105"}; // 이미 만들어진 배열
        ArrayList<String> pitches = new ArrayList<>(Arrays.asList(data)); // asList()는 이미 만들어진 배열을 List 에 추가
        List<String> pitches1 = new ArrayList<>(); //ArrayList 의 상위인 List 로 선언하면 호환성을 높일 수 있음
        // List 인터페이스로 ArrayList 객체를 참조함
        pitches.add("138"); // ArrayList 마지막에 각 값 추가 (이때 사이즈가 증가함)
        pitches.add("129");
        pitches.add("138");
        ...
        System.out.println(pitches.remove("129")); // 리스트에서 해당 항목을 삭제하고 true/false 로 반환
